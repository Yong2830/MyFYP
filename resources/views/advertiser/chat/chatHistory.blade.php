...
@section('content')
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <a href="{{ route('advertiserViewChatList') }}" class="btn btn-primary mb-3">Back to Index</a>
                <div class="card">
                    <div class="card-header">
                        @if (auth()->guard('advertiser')->check())
                            <p>Chatting with: {{ $chatList->tenant->tenant_name  }}</p>
                        @endif    
                    </div>
                ...
                <div class="card mt-3">
                    <div class="card-body">
                        <form action="{{ route('advertiserSendMessage' , $chatList->chat_list_id) }}" method="POST" id="advertiserChatForm">
                            @csrf
    ...
    <script>
        const advertiserForm = document.getElementById('advertiserChatForm');
        const chatHistoryList = document.getElementById('chatHistoryList');
        const chatListId = chatHistoryList.getAttribute('data-chat-list-id');

        advertiserForm.addEventListener('submit', function (event) {
            event.preventDefault(); // Prevent the default form submission

            // Get the form data
            const formData = new FormData(advertiserForm);
            // Assuming the form input name is 'chat_list_id'

            // Send the form data using Axios (or any other HTTP library)
            axios.post(`/advertiser/chatList/chat/${chatListId}/send`, formData)
                .then(response => {
